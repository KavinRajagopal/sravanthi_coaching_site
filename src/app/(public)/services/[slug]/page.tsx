      ...
      <section className="bg-brand-black pt-32 pb-24">
        <div className="max-w-4xl mx-auto px-6">
          <ScrollReveal>
            ...
            {service.tagline && <p className="text-brand-gold font-sans text-xs tracking-[0.3em] uppercase mb-4">{service.tagline}</p>}
            <h1 className="font-display text-5xl md:text-7xl font-light text-brand-cream leading-[1.05] mb-6">{service.title}</h1>
            <p className="text-brand-muted text-xl leading-relaxed mb-12">{service.summary}</p>
          </ScrollReveal>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-16">
            {service.format && (
              <ScrollReveal delay={0.1} className="border border-brand-border p-5">
                <p className="text-brand-gold text-xs tracking-widest uppercase mb-1 font-sans">Format</p>
                <p className="text-brand-cream">{service.format}</p>
              </ScrollReveal>
            )}
            {service.duration && (
              <ScrollReveal delay={0.2} className="border border-brand-border p-5">
                <p className="text-brand-gold text-xs tracking-widest uppercase mb-1 font-sans">Duration</p>
                <p className="text-brand-cream">{service.duration}</p>
              </ScrollReveal>
            )}
            {service.pricing?.showPrice && service.pricing?.price && (
              <ScrollReveal delay={0.3} className="border border-brand-gold p-5 bg-brand-elevated">
                ...
                <p className="text-brand-cream font-display text-2xl">{service.pricing.price}</p>
              </ScrollReveal>
            )}
          </div>
          ...
          {service.outcomes?.length > 0 && (
            <ScrollReveal delay={0.2} className="mb-16">
              <h2 className="font-display text-3xl text-brand-cream font-light mb-8">What's included</h2>
          ...
          {service.targetAudience && (
            <ScrollReveal delay={0.3} className="mb-16 border-l-2 border-brand-gold pl-6">
              <h2 className="font-display text-2xl text-brand-cream font-light mb-4">This is for you if...</h2>
              <p className="text-brand-muted leading-relaxed">{service.targetAudience}</p>
            </ScrollReveal>
          )}

          <ScrollReveal delay={0.4}>
            <Button asChild className="bg-brand-gold hover:bg-brand-gold-light text-brand-black px-10 py-6 text-sm tracking-widest uppercase rounded-none">
